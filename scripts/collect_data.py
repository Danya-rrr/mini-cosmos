...
Collects RGB frames and vehicle telemetry from CARLA simulator for training
the world model. Supports configurable episodes, traffic scenarios, and weather.
"""
...
@dataclass
class EpisodeConfig:
    """Configuration for data collection episodes."""
    num_vehicles: int = 30
...
class CARLADataCollector:
    """
    CARLA data collector for autonomous driving datasets.
    ...
    def connect(self) -> bool:
        """Connect to CARLA server and configure simulation settings."""
        try:
    ...
    def spawn_npc_vehicles(self) -> int:
        """Spawn NPC vehicles for traffic simulation."""
        spawn_points = self.world.get_map().get_spawn_points()
    ...
    def set_weather(self):
        ...
        self.current_weather_idx = random.randint(0, len(self.WEATHER_PRESETS) - 1)
        weather = self.WEATHER_PRESETS[self.current_weather_idx]
        self.world.set_weather(weather)
    
    def get_frame_data(self, frame_id: int) -> FrameData:
        """Extract telemetry data for current frame."""
    ...
    def collect_episode(self, episode_id: int) -> bool:
        ...
        print(f"\nEpisode {episode_id}: Collecting {self.config.frames_per_episode} frames...")
    ...
    def collect_dataset(self, num_episodes: int):
        ...
        print(f"\nMini-Cosmos: CARLA Data Collection")
