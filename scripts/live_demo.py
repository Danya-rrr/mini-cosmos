...
Real-time World Model inference with CARLA simulator, showing predicted
future frames alongside ground truth for live visualization.
"""
...
class LivePredictor:
    """
    Real-time frame predictor using VAE and World Model.
    
    Args:
        vae: Trained VAE model
        world_model: Trained World Model
        context_length: Number of context frames to maintain
        image_size: Size for frame processing
    ...
    def add_frame(self, image, action=None):
        """Add new frame and action to context buffer."""
    ...
    def has_enough_context(self):
        """Check if enough context frames are available."""
    ...
    def get_context_visualization(self):
        """Get last context frame for visualization."""
...
class CARLAInterface:
    """
    Interface for connecting to CARLA simulator and capturing frames.
    ...
    def get_frame(self):
        """Get latest frame and control data."""
...
def create_visualization(context_frame, predictions, ground_truth=None, fps=0, pred_step=1):
    """Create visualization canvas with context, predictions, and ground truth."""
    h, w = context_frame.shape[:2]
    num_pred = min(4, len(predictions)) if predictions else 0
    
    canvas_w = w * (2 + num_pred) if ground_truth is not None else w * (1 + num_pred)
...
def run_demo(args):
    """Run live demo with CARLA and World Model."""
