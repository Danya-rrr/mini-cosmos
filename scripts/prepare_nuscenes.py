...
Converts nuScenes dataset to training format compatible with the project's
data pipeline, extracting front camera frames and metadata.
"""
...
def prepare_nuscenes(
    input_dir='./data/external/nuscenes',
    output_dir='./data/processed/nuscenes',
    target_size=(256, 256),
    camera='CAM_FRONT'
):
    """
    ...
    Args:
        input_dir: Path to nuScenes dataset root
        output_dir: Path for processed output
        target_size: Target image size (width, height)
        camera: Camera sensor to extract (default: CAM_FRONT)
    """
    ...
    print(f"\nDone! {len(scenes)} scenes, {total_frames} frames")
